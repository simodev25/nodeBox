...
const funcPort = Number(process.env.FUNC_PORT || '8085');

const modfunctions = require.main.filename;
const modRootPath = path.join(modfunctions, '..', '..', 'functions');
...
const script = new vm.Script('\nrequire(\'functions\')(require(\'' + modPath + '\'));\n', {
    filename: modPath,
    displayErrors: true,
});
...
function modExecute(handler, req, res, end) {
    ...
    try {
        ...
        Promise.resolve(func(data,req, res))
            // Finalize
...
const server = app.listen(funcPort, () => winston.log('info', 'starting node index.js'));
